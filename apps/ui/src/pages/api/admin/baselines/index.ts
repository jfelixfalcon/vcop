...
import {
  getClusterBaselines,
  saveClusterBaseline,
  deleteClusterBaseline,
} from '../../../../lib/cluster-baselines';
...
export const GET: APIRoute = async () => {
  try {
    ...
    return new Response(JSON.stringify({ success: true, data: baselines }), {
      status: 200,
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    const updatedList = await saveClusterBaseline(
      body,
...
export const DELETE: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    const updatedList = await deleteClusterBaseline(id);
    return new Response(JSON.stringify({ success: true, data: updatedList }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
